...
image := $(namespace)/$(PRJ_NAME)
testImage := $(namespace)/$(PRJ_NAME)-testrunner
...
run-local:
	cd examples/compose && TAG=$(tag) $(composeLocal) -p $(PRJ_NAME) up -d

stop-local:
	cd examples/compose && TAG=$(tag) $(composeLocal) -p $(PRJ_NAME) stop || true
	cd examples/compose && TAG=$(tag) $(composeLocal) -p $(PRJ_NAME) rm -f || true

run:
	$(call check_var, TRITON_PROFILE \
		required to run the example on Triton.)
	cd examples/triton && TAG=$(tag) docker-compose -p $(PRJ_NAME) up -d

stop:
	$(call check_var, TRITON_PROFILE \
		required to run the example on Triton.)
	cd examples/compose && TAG=$(tag) docker-compose -p $(PRJ_NAME) stop || true
	cd examples/compose && TAG=$(tag) docker-compose -p $(PRJ_NAME) rm -f || true
